...
## Autoload: SettingsManager (wspólny)
## Wczytuje i zapisuje ustawienia wyświetlania.
...
func get_available_resolutions() -> Array[Vector2i]:
	...
	var candidates : Array[Vector2i] = [
		Vector2i(640,  480), Vector2i(800,  600), Vector2i(1024, 600),
...
func _load() -> ConfigFile:
	...
	if cfg.load(CONFIG_PATH) != OK:
		resolution      = DisplayServer.screen_get_size(0)
		window_mode_idx = 2
		return cfg
	window_mode_idx = cfg.get_value(SEC, "window_mode_idx", 2)
	resolution  = Vector2i(
		cfg.get_value(SEC, "resolution_x", DisplayServer.screen_get_size(0).x),
		cfg.get_value(SEC, "resolution_y", DisplayServer.screen_get_size(0).y)
	)
